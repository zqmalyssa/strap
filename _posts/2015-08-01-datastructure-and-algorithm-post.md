...
动态规划的无后效应，给定一个确定的状态，它的未来发展只与当前状态有关，而与过去经历的所有状态无关。（有点马尔可夫的性质在里面），如果爬楼梯有不能连续爬1步的限制，那么就不算无后性，动态方程会失效

因此，需要扩展状态的定义。状态[i,j]表示处在第i阶并未上一轮跳了j阶，j属于 {1, 2}，

当上一轮跳了1阶时，上上一轮只能选择跳2阶，即dp[i, 1] 只能从 dp[i-1, 2]转移过来

当上一轮跳了2阶时，上上一轮可以选择跳1阶或跳2阶，即dp[i, 2]可以从dp[i-2, 1]或者 dp[i-2, 2]转移过来

最终返回 dp[n, 1] + dp[n, 2]即可，两者之和代表爬到第n阶的方案总数，这边只是加了一个状态就又满足了无后效应。但很多问题具有非常严重的"有后效性"
...
对于问题1，总的来说，如果一个问题包含重叠子问题、最优子结构，并满足无后效性，那么它通常适合用动态规划求解。然而，我们很难从问题描述中直接提取出这些特性。因此我们通常会放宽条件，先观察问题是否适合使用回溯（穷举）解决。

适合用回溯解决的问题通常满足“决策树模型”，这种问题可以使用树形结构来描述，其中每一个节点代表一个决策，每一条路径代表一个决策序列。换句话说，如果问题包含明确的决策概念，并且解是通过一系列决策产生的，那么它就满足决策树模型，通常可以使用回溯来解决。在此基础上，动态规划问题还有一些判断的“加分项”。问题包含最大（小）或最多（少）等最优化描述。问题的状态能够使用一个列表、多维矩阵或树来表示，并且一个状态与其周围的状态存在递推关系。相应地，也存在一些“减分项”。问题的目标是找出所有可能的解决方案，而不是找出最优解。问题描述中有明显的排列组合的特征，需要返回具体的多个方案。

解题步骤，动态规划的解题流程会因问题的性质和难度而有所不同，但通常遵循以下步骤：描述决策，定义状态，建立dp表，推导状态转移方程，确定边界条件等。

按照课程的介绍，其实动态可以从 暴力搜索（递归） -> 记忆化搜索（递归） -> 动态规划（迭代）这样演进完成，就会很清晰了，更进一步，还有动态规划优化空间，就是把数组或矩阵的空间用量再减小

0-1背包问题是很好的动规入门问题，变种背包、完全背包（每个物品可以重复选取，只要将i-1变成i就行）、多重背包问题（）。我们可以将 0-1 背包问题看作一个由n轮决策组成的过程，对于每个物体都有不放入和放入两种决策，因此该问题满足决策树模型。该问题的目标是求解“在限定背包容量下能放入物品的最大价值”，因此较大概率是一个动态规划问题。
...
步骤1：对于每个物品来说，不放入背包，背包容量不变；放入背包，背包容量减小。由此可得状态定义：当前物品编号i和背包容量c。记为[i, c]。状态[i, c]对应的子问题，前i个物品在容量为c的背包中的最大价值，记作dp[i, c]，待求解的是dp[n, cap]。因此需要一个尺寸为(n+1)(cap+1)的二维dp表

步骤2：不放入物品i，背包容量不变，状态变化为[i-1, c]，放入物品i，背包容量减少wgt[i-1]，价值增加val[i-1]，状态变化为[i-1, c-wgt[i-1]]。最优子结构，放入物品i和不放入物品i两种方案中价值更大的那个，可以推导状态转移方程。若当前物品重量超出剩余背包容量c，则只能选择不放入背包

步骤3：当无物品或背包容量为0的时候，最大价值为0，dp的首行和首列都为0，两层循环正序遍历整个dp表就行。0-1背包问题的空间优化在正序遍历的时候，需要上方和左上方的值，可能会被覆盖了，倒序遍历的时候则不会被覆盖，所以将内循环倒序遍历就行了

不考虑时间的前提下，所有动态规划问题都可以用回溯（暴力搜索）进行求解（这个概念很重要，其实是一个递进的关系），但递归树中存在大量的重叠子问题，效率极低。通过引入记忆化列表，可以存储所有计算过的子问题的解，从而保证重叠子问题只被计算一次。记忆化搜索是一种从顶至底的递归式解法，而与之对应的动态规划是一种从底至顶的递推式解法，其如同“填写表格”一样。由于当前状态仅依赖某些局部状态，因此我们可以消除dp表的一个维度，从而降低空间复杂度。
...
可以看看零钱兑换问题，贪心算法无法保证找到全局最优解，并且有可能找到非常差的解。它更适合用动态规划解决。

一般情况下，贪心算法的适用情况分以下两种。1是可以保证找到最优解：贪心算法在这种情况下往往是最优选择，因为它往往比回溯、动态规划更高效。2是可以找到近似最优解：贪心算法在这种情况下也是可用的。对于很多复杂问题来说，寻找全局最优解非常困难，能以较高效率找到次优解也是非常不错的。
